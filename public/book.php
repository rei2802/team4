...
// Pre-fill form if logged in — guests can access freely
$userInfo = [];
...
<a href="index.php#services">Services</a>      <a href="fleets.php">Fleets</a>
  ...
  <div class="booking-wrapper">
    ...
    <div class="booking-top-bar">
      <h2>BOOK A SERVICE</h2>
        ...
        <form class="booking-form" id="bookingForm" action="submitbooking.php" method="POST" onsubmit="return checkBeforeSubmit(event)">
...
function checkBeforeSubmit(e) {
    // 1. Fetch live server-side login state directly from PHP
    const phpIsLoggedIn = <?php echo isset($_SESSION['user_id']) ? 'true' : 'false'; ?>;
    
    // 2. Fetch local storage guest values
    const isGuest = localStorage.getItem('isGuest') === 'true';
    const guestName = localStorage.getItem('guestName');
    
    // 3. Inspect the current contact person text box state
    const contactPerson = document.getElementById('contactPersonName').value.trim();

    // If logged in via account session, pass validation
    if (phpIsLoggedIn) {
        return true;
    }

    // If in guest mode OR they typed a value into the contact person name input box, pass validation
    if (isGuest || contactPerson !== '') {
        return true;
    }

    // Otherwise, block the action and direct them back to log in
    e.preventDefault();
    alert('⚠️ Please log in or continue as guest before submitting a booking.');
